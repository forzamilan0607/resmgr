package com.chris.common.utils;

import com.fasterxml.jackson.databind.ObjectMapper;

import java.io.Serializable;

/**
 * @author chris
 * @since Apr 20.18
 */
public class ResponseModel<T> implements Serializable {
    private String code;
    private String msg;
    private T data;

    public static ResponseModel getSuccessResponseModel() {
        return (new ResponseModel()).setCode("200").setMsg("success");
    }

    public static ResponseModel getFailedResponseModel() {
        return (new ResponseModel()).setCode("500").setMsg("error");
    }

    public static ResponseModel getLoginErrorResponseModel() {
        return (new ResponseModel()).setCode("401").setMsg("auth_error");
    }

    public ResponseModel() {
    }

    public ResponseModel(String code, String msg) {
        this.code = code;
        this.msg = msg;
    }

    public String getCode() {
        return this.code;
    }

    public ResponseModel setCode(String code) {
        this.code = code;
        return this;
    }

    public String getMsg() {
        return this.msg;
    }

    public ResponseModel setMsg(String msg) {
        this.msg = msg;
        return this;
    }

    public T getData() {
        return this.data;
    }

    public ResponseModel setData(T data) {
        this.data = data;
        return this;
    }

    public String toString() {
        try {
            String e = "ResponseModel{code=\'" + this.code + '\'' + ", msg=\'" + this.msg + '\'' + ", data=" + (new ObjectMapper()).writeValueAsString(this.data) + '}';
            return e;
        } catch (Exception var2) {
            var2.printStackTrace();
            return null;
        }
    }
}
